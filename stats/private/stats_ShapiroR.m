function [ stat ] = stats_ShapiroR(pred,target)
%STATS_SHAPIROR	Shapiro's R, the geometric mean of positive outcomes
%	[ stat ] = stats_ShapiroR(pred,target) 
%	
%
%	Inputs:
...
stat=exp(sum(log(pred(logical(target))))/sum(target));
