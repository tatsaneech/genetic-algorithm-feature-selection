...
%TODO: Make this check redundant by ensuring correct dimensions upstream
if size(pred,2)==size(target,1)
    pred=pred';
end

tUniq = unique(target);
if length(tUniq)==2 % Binary statcome - classification
    pred = pred > 0.5 ;
    stat = 0;
    for c=tUniq'
    ...
    fPred = interp1(xiPred,fPred,x,'linear','extrap'); 
    fPred = fPred - min(fPred) +1e-6;
