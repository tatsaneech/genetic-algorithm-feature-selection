...
elseif oSz(2) == 1
    ...
    error('GAFS:AlgoGen:TooManyTargetVectors',...
        ['The target vector has more than one vector of targets. \n' ...
...
if size(uniqOut,1) > 2
    ...
    if iscell(origOutcome)
        ...
        outcome = double(strcmp(origOutcome,uniqOut{1}));
    elseif ischar(origOutcome)
        % Pick a random outcome as the positive outcome
        warning('GAFS:AlgoGen:UnspecifiedPositiveTarget', ...
            ['The target input does not have a clear positive target.\n' ...
            'You should perhaps set the outcome to a double vector of \n' ...
        ...
        outcome = double(arrayfun(@(x) strcmp(x,uniqOut(1,:)),origOutcome));
    elseif islogical(origOutcome)
        outcome = double(outcome);
        
    elseif isdouble(origOutcome)
        % Assume higher outcome is positive class
        outcome = double(origOutcome > min(uniqOut));
    else
